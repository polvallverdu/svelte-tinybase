<script lang="ts" generics="T extends OptionalSchemas">
  ...
  // Backward checkpoints are the first element of the tuple (past checkpoints)
  const backwardIds = $derived(ids[0] ?? []);

  const CheckpointComponent = checkpointComponent ?? CheckpointView;
</script>

{#each backwardIds as checkpointId, i (checkpointId)}
